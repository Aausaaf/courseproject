...
const Login = () => {
   const login = useGoogleLogin({
    ...
    const {getLogin, loginDatas,loginErrors,setLoginErrors} = useMongoDbAuth()


 const handleSubmit = () =>{
    getLogin({email:loginData.email,password:loginData.password})
  ...
  return (<div className="login_page_main_container">
       <div className="login_page_body_contianer">
       <img style={{
            width:"53px",
            height:"53px"
         }} src={icon} alt="" />
         ...
         <h1>Login</h1>
         <h2>Don't have an account ? <span  onClick={()=>{
            navigate("/signup")
         }}>Sign up</span></h2>
         <div  style={{
            visibility:loginErrors? "visible" : "hidden"
            , marginBottom:loginErrors? "0px":"",
            marginTop:loginErrors? "6px":""
         }}  className="error">
            <p>{loginErrors}</p>
         </div>
         <form onSubmit={(e)=>{
            ...
            <div className="inputbox">
                <p>Username</p>
                <input value={loginData.username} onChange={(e)=>{
            setLoginErrors("")
